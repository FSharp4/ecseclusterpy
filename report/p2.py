...
if __name__ == "__main__":
    ...
    mean_cv_predictions = np.mean(cross_validation_mean_predictions, axis=0)
    mean_cv_stdev = np.mean(cross_validation_mean_stdevs, axis=0)
    cv_stdev = np.std(cross_validation_mean_predictions, axis=0)

    cv_means = pd.DataFrame(data=mean_cv_predictions,
                            columns=["BIC/ESS", "AIC/ESS", "Elbow"],
                            index=["Few Clusters/Low Variance", "Few Clusters/High Variance",
                                   "Many Clusters/Low Variance",
                                   "Many Clusters/High Variance"])

    cv_mean_stdevs = pd.DataFrame(data=mean_cv_stdev,
                            columns=["BIC/ESS", "AIC/ESS", "Elbow"],
                            index=["Few Clusters/Low Variance", "Few Clusters/High Variance",
                                   "Many Clusters/Low Variance",
                                   "Many Clusters/High Variance"])

    cv_stds = pd.DataFrame(data=cv_stdev,
                            columns=["BIC/ESS", "AIC/ESS", "Elbow"],
                            index=["Few Clusters/Low Variance", "Few Clusters/High Variance",
                                   "Many Clusters/Low Variance",
                                   "Many Clusters/High Variance"])

    print("Mean Cross Validation Predictions")
    print(cv_means)

    print("Mean Cross Validation Prediction Standard Deviations")
    print(cv_mean_stdevs)

    print("Standard Deviation of Mean Cross Validation Predictions")
    print(cv_stds)
    ...
    plt.figure()
    plt.bar(r, mean_cv_predictions[:, 0], yerr=mean_cv_stdev[:, 0], width=width, label='BIC/ESS')
    plt.bar(r + width, mean_cv_predictions[:, 1], yerr=mean_cv_stdev[:, 1], width=width, label='AIC/ESS')
    plt.title(f"Graph of k-fold mean predicted # clusters ({n} x {m})")
    ...
    plt.figure()
    plt.bar(r, mean_raw_predictions[:, 0], yerr=raw_prediction_stdev[:, 0], width=width, label='BIC/ESS')
    plt.bar(r + width, mean_raw_predictions[:, 1], yerr=raw_prediction_stdev[:, 1], width=width, label='AIC/ESS')
    plt.title(f"Graph of average predicted # clusters ({n} x {m})")
    plt.xticks(r + width, x)
    plt.ylabel("Predicted # Clusters")
    plt.legend()
    plt.show()
