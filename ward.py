...
def _debug_ward(data: Dataset) -> ndarray:
    n = data.features.__len__()
    clusters: list[Cluster] = [None] * n
    # for datapoint in data:
    for i in range(0, n):
        datapoint = data.features[i]
        as_array = ndarray([1, datapoint.__len__()])
        as_array[0] = [dim for dim in datapoint]
    ...
    for iteration in range(0, n - 1):
        ...
        matrix_logger.store_matrix_by_series(D[i_min], "reference_distance_vector")
        for k in range(0, n):
